...
def start_server():
    ...
    while True:
        try:
            ...
            # First message from client → attempt login
            if addr not in clients.values():
                username = authenticate_user(sock, addr, users)
                if username:
                    clients[username] = addr
                    print(f"👤 {username} connected from {addr}")
                    threading.Thread(target=handle_client, args=(sock, username, addr), daemon=True).start()
                continue
